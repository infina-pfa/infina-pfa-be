...
export class DebtController {
  ...
  async getDebts(@CurrentUser() user: AuthUser): Promise<DebtResponseDto[]> {
    const debts = await this.getDebtsUseCase.execute();

    // Filter debts by user
    const userDebts = debts.filter((debt) => debt.userId === user.id);

    return userDebts.map((debt) => DebtResponseDto.fromAggregate(debt));
  }
