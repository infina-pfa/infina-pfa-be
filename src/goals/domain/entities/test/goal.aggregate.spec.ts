...
describe('GoalAggregate', () => {
  ...
  describe('contribute method', () => {
    ...
    it('should use default description if not provided', () => {
      ...
      // Assert
      const transactions = goalAggregate.contributions;
      expect(transactions[0].props.description).toBe(`Contribution to ${mockGoalTitle}`);
    });
  ...
  describe('withdraw method', () => {
    ...
    it('should update goal progress after withdrawal', () => {
      ...
      // Assert
      expect(updateProgressSpy).toHaveBeenCalledWith(new CurrencyVO(-75, Currency.USD));
    });
    ...
    it('should use default description if not provided', () => {
      ...
      // Assert
      const transactions = goalAggregate.contributions;
      expect(transactions[0].props.description).toBe(`Withdrawal from ${mockGoalTitle}`);
    });
  ...
  describe('totalContributed computed property', () => {
    ...
    it('should handle decimal amounts correctly', () => {
      // Arrange
      goalAggregate.contribute({ amount: new CurrencyVO(100.50, Currency.USD) });
  ...
  describe('integration tests', () => {
    ...
    it('should update goal progress correctly with mixed transactions', () => {
      ...
      expect(updateProgressSpy).toHaveBeenNthCalledWith(1, new CurrencyVO(300, Currency.USD));
      
      // Second call: after withdrawal (300 - 100 = 200)
      expect(updateProgressSpy).toHaveBeenNthCalledWith(2, new CurrencyVO(200, Currency.USD));
      
      // Third call: after second contribution (200 + 200 = 400)
      expect(updateProgressSpy).toHaveBeenNthCalledWith(3, new CurrencyVO(400, Currency.USD));
    });
    ...
    it('should handle complex scenario with many transactions', () => {
      ...
      transactions.forEach(({ type, amount }) => {
        if (type === 'contribute') {
          goalAggregate.contribute({ amount: new CurrencyVO(amount, Currency.USD) });
          expectedTotal += amount;
        } else {
          goalAggregate.withdraw({ amount: new CurrencyVO(amount, Currency.USD) });
          expectedTotal -= amount;
        }
      });
    ...
    it('should maintain transaction order and properties', () => {
      ...
      expect(transactions[2].props.description).toBe(`Contribution to ${mockGoalTitle}`);
  ...
  describe('edge cases and boundary conditions', () => {
    ...
    it('should handle large amounts without overflow', () => {
      // Arrange
      const largeAmount = new CurrencyVO(Number.MAX_SAFE_INTEGER - 1, Currency.USD);

      // Act
      goalAggregate.contribute({ amount: largeAmount });

      // Assert
      expect(goalAggregate.totalContributed.value).toBe(Number.MAX_SAFE_INTEGER - 1);
    });
    ...
    it('should set proper timestamps for all transactions', () => {
      ...
      transactions.forEach(transaction => {
        ...
        expect(transaction.props.createdAt.getTime()).toBeGreaterThanOrEqual(beforeTime.getTime());
        expect(transaction.props.createdAt.getTime()).toBeLessThanOrEqual(afterTime.getTime());
        expect(transaction.props.updatedAt.getTime()).toBeGreaterThanOrEqual(beforeTime.getTime());
        expect(transaction.props.updatedAt.getTime()).toBeLessThanOrEqual(afterTime.getTime());
      });
    });
  });
});